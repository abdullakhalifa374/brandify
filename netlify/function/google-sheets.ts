...
export const handler = async (event: any) => {
  ...
  try {
    ...
    if (action === 'getDemoTemplates') {
      // ⚠️ UPDATED: Exactly matches your tab name and column count (A to G)
    ...
    if (action === 'getMarketplaceForms') {
      const response = await sheets.spreadsheets.values.get({
        ...
        range: 'Sheet1!A:H', // We will update this later when we do Marketplace!
      });
      const rows = response.data.values || [];
      return { statusCode: 200, headers, body: JSON.stringify({ data: rows }) };
    }
    ...
    if (action === 'getClientData' && email) {
      const response = await sheets.spreadsheets.values.get({
        ...
        range: 'Clients!A:J', // Make sure your main sheet tab is actually named "Clients"
      });
